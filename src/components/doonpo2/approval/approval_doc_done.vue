...
import axios from 'axios'
  ...
  export default {
    ...
    methods:{
        async fetchData(){
          ...
          try {
            ...
            if(this.$route.query.jgcd == "주"){
              const response = await axios.get(`${this.$D2ApiPath}/api/inspection/getLineTargetListWeek?date=` + this.$route.query.workdate, { headers: { Authorization: token } } );
              this.tableItems = response.data 
              this.loading=false;
            }else{
              const response = await axios.get(`${this.$D2ApiPath}/api/inspection/getLineTargetList?date=` + this.$route.query.workdate, { headers: { Authorization: token } } );
              this.tableItems = response.data 
              this.loading=false;
            }
          } catch (error) {
        ...
        async getApproved(){
          const token = localStorage.getItem('token')
          const response = await axios.get(`${this.$D2ApiPath}/api/approval/doc?date=` + this.$route.query.workdate+'&jgcd='+ this.$route.query.jgcd, { headers: { Authorization: token } } )
