...
//      apexchart: VueApexCharts,
    ...
    methods:{
      retrieveCalendar(){
        ...
        axios.get(`${this.$D2ApiPath}/api/approval/wait?from=` + this.getFrom +'&to='+ this.dateweek.replace(/-/g, ""), { headers: { Authorization: token } } )
          .then(response => {
      ...
      save(){
        ...
        const token = localStorage.getItem('token')
            axios.patch(`${this.$D2ApiPath}/api/approval/all/wait`,arr, { headers: { Authorization: token } } )
            .then(response => {
