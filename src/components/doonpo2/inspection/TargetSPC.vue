  ...
  <div id="app">
    <v-app id="inspire" >
      <!---
                    ...
                    -->
                    <v-dialog v-model="modalVisible" v-if="modalVisible" 
        max-width="480"
        transition="dialog-bottom-transition">
                      <domodal :line="modalData.line" @close-modal="closeModal"
                                :part="modalData.part"
                                :cpid="modalData.cpid" 
                                :workdate="modalData.workdate"
                                :zone="modalData.zone"></domodal>
                    </v-dialog>
                      ...
                      <v-col md="7" class="pr-0">
                       ...
                       <v-col class="align-self-center" md="4">
                        <template v-if="isPercent">
                          ...
                          <v-progress-linear :value="getNum(percentItem.DONE_CNT/this.tableItems.length)*100"  :color="getSliderColor"></v-progress-linear>
                        </template>
                      </v-col>
                  </v-row>
             </v-app-bar> 
          ...
          <v-card :style="{borderLeft: getLeftColor[index]}" 
            class="mx-auto page-content"
            outlined
            elevation="2" :tabindex="index" :id="getStringId[index]" >
            <!--
                  ...
                  <v-row class="pa-0 ma-0">
                    <v-col class="pa-0 ma-0">
                      <v-row class="pa-0 ma-0">
                        <v-col  style="text-align:right" >
                          <template v-if="item.IS_DONE!=1">
                            <v-btn  tile @click="openModal(item.WCEN,item.MACO,item.INDEX_KEY)">
                                <v-icon left>mdi-lead-pencil</v-icon>검사시작
                            </v-btn>
                          </template>
                          <template v-else>
                              <v-btn  tile outlined color="success" @click="openModal(item.WCEN,item.MACO,item.INDEX_KEY)">
                                  <v-icon left>mdi-lead-pencil</v-icon>수정
                              </v-btn>
                          </template>
                        </v-col>
                      ...
                      <v-row class="pa-0 ma-0" v-if="item.SCGU==='Y'">
                        <v-col  style="text-align:right" >
                          <v-card-text class="text--primary"><v-icon color="info">mdi-check-circle</v-icon><span class="text--primary"> 특별특성</span></v-card-text>
                        </v-col>
                      </v-row>
                    </v-col>
                  </v-row>
...
import DoModal from './detail/PatrolDo_Modal';
//import SimpleKeyboard from './keyboard/NumericKeyboard';
  ...
  export default {
    ...
    methods: {
      ...
      openModal (qLINE,qPART,qCPID) {
        const token = localStorage.getItem('token')
        axios.patch(`${this.$D2ApiPath}/api/inspection/todo/starttime/`+ qCPID,this.$store.state.user , { headers: { Authorization: token } })
        this.modalData.line = qLINE
        this.modalData.part = qPART
        this.modalData.cpid = qCPID
        this.modalData.workdate = this.date
        this.modalData.zone = this.$route.query.zone
        this.modalVisible=true // emit the event to the bus
      },
      onChange(input) {
        this.input = input;
      },
      ...
      callDetailView(qLINE,qPART,qCPID) {
        ...
        */
        //this.$router.push({path: '/dp2/inspection/tl/ts/do', name: '결과입력(둔포2)', query:{line: qLINE, part: qPART, cpid: qCPID, workdate: this.date, zone: this.$route.query.zone}});
      },
      getNum(val) {
          val = +val || 0
          return val;
      ...
      Initialize(){
        const token = localStorage.getItem('token')
        //  axios.get(`${this.$D2ApiPath}/api/inspection/target/percent/` + this.$route.query.line +'/'+this.$route.query.part+'/'+this.$route.query.workdate.replace(/-/g, "")+'/'+this.$route.query.zone, { headers: { Authorization: token } }) 
        //       .then(response => {
        ...
        //       })
        axios.get(`${this.$D2ApiPath}/api/inspection/target/spclist/` + this.$route.query.line +'/'+this.$route.query.part+'/'+this.$route.query.workdate.replace(/-/g, "")+'/'+this.$route.query.zone, { headers: { Authorization: token } }) 
            .then(response => {
              this.tableItems = response.data.recordset     // 반환되는 값을 toDoItems에 저장한다.
              this.percentItem=response.data.output
              this.isPercent = response.data.output ? true : false
              //console.log(response.data.output.DONE_CNT)
        ...
        //this.$ref.mybutton.$el.scrollIntoView({ behavior: 'smooth' });
      },
    },
    created(){
      this.Initialize()
    ...
    updated(){
      //this.$refs['content-250537'].scrollIntoView()
      //this.$refs.mybutton.scrollIntoView()


      // this.$refs=el => {
      //   console.log( el )//['#250537']
      // }
      // Object.keys(this.$refs).forEach(el => {
      //   console.log( this.$refs['#250537'])
      //   console.log(el)
      // })
    },
    computed: {
      getStringId(){
        return this.tableItems.map(function(item) {
             // if(item) return 'btn2'
             return 'content-'+item.INDEX_KEY
          });
      },
      getSliderColor(){
         var cal = this.percentItem.DONE_CNT/this.tableItems.length
         return (cal< 0.25) ? 'red' : (cal< 0.5) ? 'orange' : (cal< 0.75) ? 'indigo' : 'green'
      },
      getHint(){
        return '완료 '+this.percentItem.DONE_CNT+' / 대상 '+this.tableItems.length
      },
      getShowExpand(){
            return this.tableItems.map(function(item) {
                return item.BOOL
