            ...
            <v-app-bar :height="getMobileHeight" app  elevation="1"  >
              <!--
                    ...
                    <v-row class="ma-0 pa-0">
                      ...
                      <v-btn class="align-self-center" elevation="0" text color="indigo" @click="$router.push({name: '조회/입력(수원)', params:{jgcd: 1}})"><v-icon>mdi-arrow-left</v-icon></v-btn>
  ...
  export default {
    ...
    methods: {
      ...
      callDetailView(qLINE,qPART,qCPID) {
        ...
        axios.patch(`${this.$D2ApiPath}/api/inspection/todo/starttime/week/`+ qCPID,this.$store.state.user , { headers: { Authorization: token } })
        /*
        ...
        */
        this.$router.push({path: '/dp2/inspection/tl/ts/do/week', query:{line: qLINE, part: qPART, cpid: qCPID, workdate: this.date, zone: this.$route.query.zone}});
      },
      getNum(val) {
          val = +val || 0
          return val;
    ...
    created(){
      const token = localStorage.getItem('token')
       axios.get(`${this.$D2ApiPath}/api/inspection/target/percent/week/` + this.$route.query.line +'/'+this.$route.query.part+'/'+this.$route.query.workdate.replace(/-/g, "")+'/'+this.$route.query.zone, { headers: { Authorization: token } }) 
            .then(response => {
        ...
        axios.get(`${this.$D2ApiPath}/api/inspection/target/spclist/week/` + this.$route.query.line +'/'+this.$route.query.part+'/'+this.$route.query.workdate.replace(/-/g, "")+'/'+this.$route.query.zone, { headers: { Authorization: token } }) 
            .then(response => {
