...
export default {
  ...
  created(){
    const token = localStorage.getItem('token')
      axios.get(`${this.$D1ApiPath}/api/inspection/todo/list/week/` + this.cpid, { headers: { Authorization: token } }) 
          .then(response => {
      ...
      saveandcall(qLINE,qPART,qCPID){
        ...
        const token = localStorage.getItem('token')
          //console.log(qCPID +'/' + this.$route.query.workdate +'/'+this.$route.query.zone+'/'+this.input+'/'+this.toggle_val+'/'+this.etcinput)
            ...
            axios.patch(`${this.$D1ApiPath}/api/inspection/todo/endtime/week/`+qCPID +'?pINPUT='+this.input+'&pIS_OK='+this.toggle_val+'&pETC='+this.etcinput,this.$store.state.user, { headers: { Authorization: token } }) 
