  ...
  export default {
    ...
    created(){
      const token = localStorage.getItem('token')
        axios.get(`${this.$D2ApiPath}/api/inspection/todo/list/` + this.$route.query.cpid, { headers: { Authorization: token } }) 
            .then(response => {
        ...
        saveandcall(qLINE,qPART,qCPID){
          ...
          const token = localStorage.getItem('token')
            //console.log(qCPID +'/' + this.$route.query.workdate +'/'+this.$route.query.zone+'/'+this.input+'/'+this.toggle_val+'/'+this.etcinput)
              ...
              axios.patch(`${this.$D2ApiPath}/api/inspection/todo/endtime/`+qCPID +'?pINPUT='+this.input+'&pIS_OK='+this.toggle_val+'&pETC='+this.etcinput,this.$store.state.user, { headers: { Authorization: token } }) 
              ...
              this.$router.push({path: '/dp2/inspection/tl/ts', name: '공정조회(둔포2)', query:{line: qLINE, part: qPART,workdate: this.$route.query.workdate, zone: this.$route.query.zone}});
        },
        onChange(input) {
        this.input = input;
