...
import axios from 'axios'
  ...
  export default {
    ...
    created(){
        axios.get('http://192.168.28.16:3000/api/inspection/getLineTargetList/' + this.date.replace(/-/g, "")) // http://localhost:3000/api/inspection/GetDoList/에 get call을 한다.
            .then(response => {
      ...
      methods:{
        retrieveCalendar(){
          ...
          axios.get('http://192.168.28.16:3000/api/inspection/getLineTargetList/' + pickdate.replace(/-/g, "")) // http://localhost:3000/api/inspection/GetDoList/에 get call을 한다.
            .then(response => {
              this.tableItems = response.data     // 반환되는 값을 toDoItems에 저장한다.
              //console.log(response.data.date);
            })
