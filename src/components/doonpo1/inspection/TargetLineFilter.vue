  ...
  export default {
    ...
    methods:{
        ...
        retrieveCalendarWeek(){
          ...
          axios.get(`${this.$D1ApiPath}/api/inspection/getLineTargetListWeek?date=` + firstday.replace(/-/g, ""), { headers: { Authorization: token } } )
            .then(response => {
        ...
        callTargetSPC(qLINE,qPART,pWORK_DATE,pZONE) {
         ...
         // let pickedPART=this.fPART;
            ...
            this.$router.push({path: '/dp1/inspection/tl/ts',  query:{line: qLINE, part: qPART,workdate: pWORK_DATE.replace(/-/g, ""), zone: pZONE}});
            //console.log(fLINE)
        },
        callTargetSPC_Week(qLINE,qPART,pWORK_DATE,pZONE) {
            this.$router.push({path: '/dp1/inspection/tl/ts/week', query:{line: qLINE, part: qPART,workdate: pWORK_DATE.replace(/-/g, ""), zone: pZONE}});
        },
        updateIS_MANUAL(){
            const token = localStorage.getItem('token')
            axios.patch(`${this.$D1ApiPath}/api/inspection/tl/manual?index_key=` + this.editedItem.INDEX_KEY+'&pIS_MANUAL='+this.editedItem.IS_MANUAL, { headers: { Authorization: token } } )
            .then(() => {
              this.retrieveCalendar() 
            })
        },
        updateIS_MANUAL_Week(){
            const token = localStorage.getItem('token')
            axios.patch(`${this.$D1ApiPath}/api/inspection/tl/manual/week?index_key=` + this.editedItem.INDEX_KEY+'&pIS_MANUAL='+this.editedItem.IS_MANUAL, { headers: { Authorization: token } } )
            .then(() => {
              this.retrieveCalendarWeek() 
            })
        },
