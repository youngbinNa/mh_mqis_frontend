...
import axios from 'axios'
  ...
  export default {
    ...
    created(){
        ...
        axios.get('http://192.168.28.16:3000/api/inspection/getLineTargetList?date=' + this.date.replace(/-/g, "")+'&zone=' + this.zones[this.dayornot].value ) // http://localhost:3000/api/inspection/GetDoList/에 get call을 한다.
            .then(response => {
      ...
      methods:{
        retrieveCalendar(){
          ...
          axios.get('http://192.168.28.16:3000/api/inspection/getLineTargetList?date=' + pickdate.replace(/-/g, "")) // http://localhost:3000/api/inspection/GetDoList/에 get call을 한다.
            .then(response => {
              this.tableItems = response.data     // 반환되는 값을 toDoItems에 저장한다.
              //console.log(response.data.date);
            })
        ...
        callTargetSPC(fLINE,fPART) {
         ...
         // let pickedPART=this.fPART;
            ...
            this.$router.push({path: '/sw/inspection/tl/ts', name: '공정조회(수원)', query:{line: fLINE, part: fPART}});
            //console.log(fLINE)
        },
        setSelected(selectedzone) {
          //  trigger a mutation, or dispatch an action  
          let pickdate=this.date;
          axios.get('http://192.168.28.16:3000/api/inspection/getLineTargetList?date=' + pickdate.replace(/-/g, "") +'&zone='+ selectedzone) // http://localhost:3000/api/inspection/GetDoList/에 get call을 한다.
            .then(response => {
              this.tableItems = response.data     // 반환되는 값을 toDoItems에 저장한다.
              //console.log(response.data.date);
            })
