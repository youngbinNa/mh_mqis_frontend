              ...
              <div>
                <v-toolbar dense flat>
                  ...
                  <v-col class="ma-0 pa-0 justify-end"  style="text-align:right">
                    <v-btn outlined tile class="text-h6" color="primary" link :to="{path: '/SW/approval/wait'}" ><v-icon left>mdi-format-list-bulleted</v-icon> 목록</v-btn>
  ...
  export default {
    ...
    methods:{
        fetchData(){
          ...
          try {
            ...
            if(this.$route.query.jgcd == "주"){
              axios.get(`${this.$D1ApiPath}/api/inspection/getLineTargetListWeek?date=` + this.$route.query.workdate, { headers: { Authorization: token } } )
              .then(response => {
                this.tableItems = response.data 
                this.loading=false;
              })
            }else{
              axios.get(`${this.$D1ApiPath}/api/inspection/getLineTargetList?date=` + this.$route.query.workdate, { headers: { Authorization: token } } )
              .then(response => {
                this.tableItems = response.data 
                this.loading=false;
              })
         ...
         getApproved(){
          const token = localStorage.getItem('token')
          axios.get(`${this.$D1ApiPath}/api/approval/doc?date=` + this.$route.query.workdate+'&jgcd='+ this.$route.query.jgcd, { headers: { Authorization: token } } )
          .then(response => {
        ...
        retrieveCalendar(){
          ...
          const token = localStorage.getItem('token')
          axios.get(`${this.$D1ApiPath}/api/inspection/getLineTargetList?date=` + this.date.replace(/-/g, ""), { headers: { Authorization: token } } )
            .then(response => {
        ...
        callTargetSPC(qLINE,qPART,pWORK_DATE,pZONE,pJGCD) {
         ...
         // let pickedPART=this.fPART;
            ...
            this.$router.push({path: '/dp2/approval/doc/detail',  query:{line: qLINE, part: qPART,workdate: pWORK_DATE.replace(/-/g, ""), zone: pZONE, jgcd: pJGCD}});
            //console.log(fLINE)
        },
        getNum(val) {
          val = +val || 0
        ...
        save(){
          ...
          const token = localStorage.getItem('token')
              axios.patch(`${this.$D1ApiPath}/api/approval/detail/doc`,arr, { headers: { Authorization: token } } )
              .then(response => {
                alert(response.data.msg) 
                this.$router.push({path: '/dp2/approval/wait'});
              })
