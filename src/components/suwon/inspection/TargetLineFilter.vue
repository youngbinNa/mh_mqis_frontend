  ...
  export default {
    ...
    methods:{
        ...
        retrieveCalendarWeek(){
          ...
          axios.get(`${this.$SuwonApiPath}/api/inspection/getLineTargetListWeek?date=` + firstday.replace(/-/g, ""), { headers: { Authorization: token } } )
            .then(response => {
        ...
        updateIS_MANUAL_Week(){
            const token = localStorage.getItem('token')
            axios.patch(`${this.$SuwonApiPath}/api/inspection/tl/manual/week?index_key=` + this.editedItem.INDEX_KEY+'&pIS_MANUAL='+this.editedItem.IS_MANUAL, { headers: { Authorization: token } } )
            .then(() => {
              this.retrieveCalendarWeek() 
            })
        },
