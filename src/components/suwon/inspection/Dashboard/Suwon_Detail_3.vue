     ...
     mounted() {
      ...
      methods:{
        ...
        getAxiosDataDay(){
            ...
            axios.get(this.workplace+'/api/dashboard/get3?from=' + this.date[0].replace(/-/g, "")+'&to='+ this.date[1].replace(/-/g, "")+'&zone='+ this.filter, { headers: { Authorization: token } } )
            .then(response => {
        ...
        getAxiosDataWeek(){
            const token = localStorage.getItem('token')
            axios.get(this.workplace+'/api/dashboard/get3/week?from=' + this.date[0].replace(/-/g, "")+'&to='+ this.date[1].replace(/-/g, "")+'&zone='+ this.filter, { headers: { Authorization: token } } )
            .then(response => {
                var str = response.data
                //let workdate = str.map(str => str.WORK_DATE.replace(/(\d{4})(\d{2})(\d{2})/g, '$1-$2-$3'));
                let workdate = str.map(str => str.WORK_DATE);
