<template>
  <v-app id="inspire" >
    ...
    <v-card>
      ...
      <!--
    <p>
  <input type="file" accept="image/*" ref="myFile"
   @change="previewFile">
  </p>
  <div v-if="imgsrc">
  ...
  data () {
    return {
        ...
        headers: [
          ...
          { text: '시료수', value: 'SIZE',width: "100px" },
          { text: '검사포인트', value: 'POINT',width: "100px" },
          ...
          { text: '결과 필수', value: 'IS_INPUT',width: "80px" },
          ...
          { text: '비고', value: 'REM2',width: "200px" },
        ],
        editedIndex: -1,
        editedItem: {
          WCEN: '',
  ...
  watch: {
   ...
   methods: {
    gettableItems(){
       ...
       axios.get(`${this.$SuwonApiPath}/api/manage/item/list`, { headers: { Authorization: token } }) 
        .then(response => {
    ...
    save () {
      if (this.editedIndex > -1) { //수정
        ...
        axios.patch(`${this.$SuwonApiPath}/api/manage/item/UPD`,{ items: this.editedItem} )
          .then(r => {
            alert(r.data.output.outmsg) //(이)가 정상 처리 되었습니다.
          })
      } else {//등록
        this.tableItems.push(this.editedItem) 
        axios.patch(`${this.$SuwonApiPath}/api/manage/item/INS`,{ items: this.editedItem} )
          .then(r => {
            alert(r.data.output.outmsg) //(이)가 정상 처리 되었습니다.
          })
      }
    ...
    submit() {
      ...
      const token = localStorage.getItem('token')
      axios.post(`${this.$SuwonApiPath}/api/manage/item/upload`, formData, {  
          headers: {
                    Authorization: token,
