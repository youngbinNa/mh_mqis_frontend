...
<div>
  <v-container style="padding:0;" app>
        <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card>
              <v-card-title class="headline">{{this.selectedRows.length}}개 문서를 일괄결재 하시겠습니까?</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
    ...
    <v-toolbar flat>
                    <v-col>
                      <v-toolbar-title>결재대기</v-toolbar-title>
                    </v-col>
                    <v-col>
                      <v-text-field
                        v-model="search"
                    ...
                    <v-col  class="justify-end" style="text-align:right">
                        <v-btn @click="dialogDelete = true" tile outlined color="primary"><v-icon left>mdi-draw</v-icon> 일괄결재</v-btn>
                    </v-col>
        </v-toolbar>
                    ...
                    <v-col class="px-4" style="text-align:center;">
                        <v-data-table id="approvTable"
                          ...
                          <template v-slot:item.WORK_DATE="{ item }">
                            ...
                            :to="{path: '/SW/approval/doc',name: '결재(수원)',  query:{workdate: item.WORK_DATE, zone: item.ZONE, jgcd: item.JGCD, ok: item.OK_NUM, yet: item.YET_NUM, def: item.DEF_ITEMS}}"  >{{ getDateFormat(item.WORK_DATE) }}</v-btn>
                          </template>
                          ...
                          <template v-slot:item.STATUS>
                            <span class="ma-0 text-h6" >*결재 대기중</span>
                          </template>
                      </v-data-table>
                    </v-col>
                </v-row>
...
//      apexchart: VueApexCharts,
    ...
    methods:{
      retrieveCalendar(){
        ...
        axios.get(`${this.$SuwonApiPath}/api/approval/wait?from=` + this.getFrom +'&to='+ this.dateweek.replace(/-/g, ""), { headers: { Authorization: token } } )
          .then(response => {
      ...
      save(){
        if(!this.selectedRows.length) return alert('문서를 1개 이상 선택해주세요.')
        var arr= {
          seqArray: this.selectedRows,
        ...
        //console.log(arr.seqArray)
        //console.log(JSON.stringify(selarray))
        const token = localStorage.getItem('token')
            axios.patch(`${this.$SuwonApiPath}/api/approval/all/wait`,arr, { headers: { Authorization: token } } )
            .then(response => {
